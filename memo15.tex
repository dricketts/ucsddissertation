...
discuss engineering trade-offs. We also discuss discrepancies between our
...
$Q~\entails \Time{} = 0~\wedge~P~\wedge~D~\wedge~0 \leq \tlanextvar{\Time{}} \leq \Delta\rightarrow \Next{P}$
\item
\label{thm:sys-ind-cont}
$Q~\entails \Time \leq \Delta~\wedge~0 \leq \tlanextvar{\Time} \leq \Time~\wedge~P~\wedge~\Continuous{(\W)} \rightarrow \Next{P}$
\end{enumerate}
then
\[
...
\wedge & \V\tlaprime = \V \\
\wedge & 0 \leq \tlanextvar{\Time{}} \leq \Delta \\
...
\V \leq \Vmax & \vdash \left[
\begin{array}{ll}
& \Time \leq \Delta \wedge 0 \leq \Time\tlaprime \leq \Time \\
...
Our description of the autopilot architecture in
...
Future work can investigate closing this formal gap using Coq's library for
...
the quadcopter to go right up to the provided height or velocity bounds.
...
even if the attitude of the quadcopter is not upward.  As a result, if the
...
\paragraph*{Global state and unconstrained transitions}
...
benefits outweigh the drawbacks. However, more significantly,
