...
the Lie derivative of $B$ along the vector field of that system is negative
at $B = 0$.\footnote{The Lie derivative of $B$ along the vector field
  $f(\vecbold{x})$ is defined by $\nabla B \dotprod f$ and gives the rate
  of change along solutions of the system $\dt{\vecbold{x}} =
...
control barrier functions, a technique for synthesizing a controller from a
...
It is important to note The only formal implementation of any of the
...
Finally, some of the above approaches,
including~\cite{prajna04barrier,kong2013barrier} also have generalizations
