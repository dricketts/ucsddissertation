...
% Do we present the control law here or derive it in the barrier certificate section
% Probably we leave it for later sections
...
Theorem~\ref{thm:barrier-smpl-weak} gives an immediate adaptation of
...
% TODO: still need to cite exponential barrier certificate papers to
% justify not putting a proof in the paper.
...
We now take a two step process to prove the invariance of $x \leq K$ for
...
% TODO make the connection between this assumption and a concrete control
% law
\begin{assumption}
For any $x,v \in \R$,
\[
...
Finally, we provide conditions on $\umaxc$ to ensure that
...
\label{lem:pw-deriv}
\end{lemma}
%TODO figure out if this is novel
